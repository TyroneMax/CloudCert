...
- **副标题**：简短描述产品价值（一句话概括）；移动端/平板居中，laptop 及以上左对齐
- **CTA 区域**：
  - 主要："Join the Waitlist" 按钮（滚动到 CTA 区域，上线前）/ "Start Practicing Free" 按钮（上线后）
...
- 详细设计见 [design-certifications-section.md](design-certifications-section.md)
...
### 6. Pricing Section（Donate）

- **标题**："Support CloudCert"（支持 CloudCert）
- **副标题**：登录得 1 个免费题库，捐赠解锁更多
- **档位**：Free（1 题库）、Thanks（$4.99）、Support（$9.99）、Sponsor（$49.99），均为一次性捐赠
...
使用折叠面板（Accordion）展示常见问题，与当前业务模型一致：

- CloudCert 适合哪些人？
- 免费版包含哪些内容？（登录后 1 个完整题库）
- 题库内容来源是什么？
- 支持哪些语言？
- 如何解锁更多题库？（捐赠档位：Thanks / Support / Sponsor）
- 题库会定期更新吗？

### 9. Waitlist（候补名单）

在产品正式上线前，**仅 CTA Section** 展示 Waitlist 邮箱收集表单；Hero Section 主 CTA 为「Join the Waitlist」按钮，点击滚动至 CTA 区域。
