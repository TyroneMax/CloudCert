...
export default function LoginPage() {
  ...
  const redirectTo = redirect && redirect.startsWith("/") ? redirect : `/${locale}/dashboard`;
  ...
  async function handleGoogleLogin() {
    ...
    try {
      ...
      if (data?.url) {
        ...
        setError("Could not initiate sign in. Please try again.");
        setLoading(false);
      }
    } catch (err) {
  ...
  async function handleEmailLogin(e: React.FormEvent) {
    ...
    // Use hard redirect so cookies are sent and middleware sees the session
      ...
      <div className="w-full max-w-md">
        ...
        <div className="rounded-2xl border border-border/60 bg-card p-8 shadow-sm">
          ...
          <form onSubmit={handleEmailLogin} className="space-y-4">
            ...
            <div>
              <div className="mb-1.5 flex items-center justify-between">
                ...
                <Link href={`/${locale}/auth/forgot-password${redirect ? `?redirect=${encodeURIComponent(redirect)}` : ""}`} className="text-xs text-blue-600 hover:text-blue-800">{t("forgotPassword")}</Link>
              </div>
              <input
                id="password"
                type="password"
          ...
          <p className="mt-6 text-center text-sm text-muted-foreground">
            {t("noAccount")}{" "}
            <Link href={`/${locale}/auth/register${redirect ? `?redirect=${encodeURIComponent(redirect)}` : ""}`} className="font-medium text-blue-600 hover:text-blue-800">
              {t("signUp")}
            </Link>
          </p>
        </div>
