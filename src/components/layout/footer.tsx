...
export function Footer() {
  ...
  const footerLinks = {
    [t("product")]: [
      ...
      { label: t("roadmap"), href: `/${locale}/roadmap` },
    ],
    [t("company")]: [
      { label: t("about"), href: "#" },
        ...
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-5">
          <div className="lg:col-span-2">
            <Link href={`/${locale}`} className="flex items-center gap-2" aria-label="CloudCert Home">
              ...
              <span className="text-lg font-bold tracking-tight">CloudCert</span>
            </Link>
            <p className="mt-4 max-w-xs text-sm leading-relaxed text-muted-foreground">{t("description")}</p>
          </div>
